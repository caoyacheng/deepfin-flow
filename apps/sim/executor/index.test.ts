...
describe("Executor", () => {
  ...
  describe("initialization", () => {
    it.concurrent(
      "should create an executor instance with legacy constructor format",
      () => {
    ...
    it.concurrent(
      "should create an executor instance with new options object format",
      () => {
        const workflow = createMinimalWorkflow();
        const initialStates = {
    ...
    it.concurrent(
      "should handle legacy constructor with individual parameters",
      () => {
        const workflow = createMinimalWorkflow();
        const initialStates = {
          block1: { result: { value: "Initial state" } },
  ...
  describe("workflow validation", () => {
    ...
    it.concurrent(
      "should throw error for workflow without starter block",
      () => {
        const workflow = createMinimalWorkflow();
        workflow.blocks = workflow.blocks.filter(
    ...
    it.concurrent(
      "should throw error for workflow with disabled starter block",
      () => {
        const workflow = createMinimalWorkflow();
    ...
    it.concurrent(
      "should throw error if starter block has incoming connections",
      () => {
        const workflow = createMinimalWorkflow();
    ...
    it.concurrent(
      "should throw error if starter block has no outgoing connections",
      () => {
        const workflow = createMinimalWorkflow();
    ...
    it.concurrent(
      "should throw error if connection references non-existent source block",
      () => {
        const workflow = createMinimalWorkflow();
        workflow.connections.push({
    ...
    it.concurrent(
      "should throw error if connection references non-existent target block",
      () => {
        const workflow = createMinimalWorkflow();
        workflow.connections.push({
          source: "starter",
  ...
  describe("workflow execution", () => {
    it.concurrent(
      "should execute workflow and return ExecutionResult",
      async () => {
        const workflow = createMinimalWorkflow();
        const executor = new Executor(workflow);
    ...
    it.concurrent(
      "should handle streaming execution with onStream callback",
      async () => {
        const workflow = createMinimalWorkflow();
        const mockOnStream = vi.fn();
    ...
    it.concurrent(
      ...
      async () => {
        ...
        const createContextSpy = vi.spyOn(
          executor as any,
  ...
  describe("debug mode", () => {
    ...
    it.concurrent(
      "should handle continue execution in debug mode",
      async () => {
        const workflow = createMinimalWorkflow();
        const executor = new Executor(workflow);
        ...
        const result = await executor.continueExecution(
          ["block1"],
          mockContext
        );
  ...
  describe("block output handling", () => {
    ...
    it.concurrent("should handle error outputs correctly", () => {
      ...
      const extractErrorMessage = (executor as any).extractErrorMessage.bind(
        executor
      );
  ...
  describe("error handling", () => {
    it.concurrent(
      "should activate error paths when a block has an error",
      () => {
        ...
        const activateErrorPath = (executor as any).activateErrorPath.bind(
          executor
        );
    ...
    it.concurrent(
      "should not activate error paths for starter and condition blocks",
      () => {
        const workflow = createWorkflowWithErrorPath();
        const executor = new Executor(workflow);
  ...
  describe("dependency checking", () => {
    it.concurrent(
      "should handle multi-input blocks with inactive sources correctly",
      () => {
        ...
        const checkDependencies = (executor as any).checkDependencies.bind(
          executor
        );
    ...
    it.concurrent(
      "should prioritize special connection types over active path check",
      () => {
        const workflow = createMinimalWorkflow();
        const executor = new Executor(workflow);
        const checkDependencies = (executor as any).checkDependencies.bind(
        ...
        const errorConnections = [
          { source: "block2", target: "block3", sourceHandle: "error" },
        ];
        ...
        const errorDepsResult = checkDependencies(
          errorConnections,
          new Set(["block2"]),
          mockContext
        );
        ...
        const loopDepsResult = checkDependencies(
          loopConnections,
          new Set(["block2"]),
          mockContext
        );
    ...
    it.concurrent(
      "should handle router decisions correctly in dependency checking",
      () => {
        const workflow = createMinimalWorkflow();
        const executor = new Executor(workflow);
        const checkDependencies = (executor as any).checkDependencies.bind(
        ...
        const selectedConnections = [
          { source: "router1", target: "target1", sourceHandle: "source" },
        ];
        const selectedResult = checkDependencies(
          selectedConnections,
          executedBlocks,
          mockContext
        );
    ...
    it.concurrent(
      "should handle condition decisions correctly in dependency checking",
      () => {
        ...
        const trueResult = checkDependencies(
          trueConnections,
          executedBlocks,
          mockContext
        );
        ...
        const falseResult = checkDependencies(
          falseConnections,
          executedBlocks,
          mockContext
        );
    ...
    it.concurrent(
      "should handle regular sequential dependencies correctly",
      () => {
        const workflow = createMinimalWorkflow();
        const executor = new Executor(workflow);
        const checkDependencies = (executor as any).checkDependencies.bind(
        ...
        const normalConnections = [
          { source: "block1", target: "block2", sourceHandle: "source" },
        ];

        // Without error
        const normalResult = checkDependencies(
          normalConnections,
          executedBlocks,
          mockContext
        );
        ...
        const errorResult = checkDependencies(
          normalConnections,
          executedBlocks,
          mockContext
        );
  ...
  describe("workflow cancellation", () => {
    it.concurrent(
      "should set cancellation flag when cancel() is called",
      () => {
        const workflow = createMinimalWorkflow();
        const executor = new Executor(workflow);
    ...
    it.concurrent(
      "should handle cancellation in debug mode continueExecution",
      async () => {
        const workflow = createMinimalWorkflow();
        const executor = new Executor(workflow);
    ...
    it.concurrent(
      "should prevent new execution on cancelled executor",
      async () => {
        const workflow = createMinimalWorkflow();
        const executor = new Executor(workflow);
    ...
    it.concurrent(
      "should return cancelled result when cancellation flag is checked",
      async () => {
        const workflow = createMinimalWorkflow();
        const executor = new Executor(workflow);
