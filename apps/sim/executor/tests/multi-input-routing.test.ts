...
describe("Multi-Input Routing Scenarios", () => {
      ...
      blocks: [
        ...
        },
        {
          ...
          config: {
            ...
            params: {
              prompt:
                "if the input is x, go to function 1.\notherwise, go to function 2.\ny",
              model: "gpt-4o",
            },
          },
          inputs: {},
  ...
  it("should handle multi-input target when router selects function-1", async () => {
    ...
    const agent1Connections = workflow.connections.filter(
      (conn) => conn.target === "agent-1"
    );
  ...
  it("should verify the dependency logic for inactive sources", async () => {
    ...
    const function1DepMet = checkDependencies(
      function1Connection,
      context.executedBlocks,
      context
    );
    ...
    const function2DepMet = checkDependencies(
      function2Connection,
      context.executedBlocks,
      context
    );
    ...
    const bothDepMet = checkDependencies(
      bothConnections,
      context.executedBlocks,
      context
    );
