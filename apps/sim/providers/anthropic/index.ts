...
export const anthropicProvider: ProviderConfig = {
  ...
  executeRequest: async (
    ...
    if (!request.apiKey) {
      throw new Error("请输入Anthropic的API密钥");
    }
        ...
        if (filteredTools?.length) {
          ...
          if (typeof tc === "object" && tc !== null) {
            if (tc.type === "tool") {
              toolChoice = tc;
              logger.info(
                `Using Anthropic tool_choice format: force tool "${tc.name}"`
              );
            } else {
              // Default to auto if we got a non-Anthropic object format
              toolChoice = "auto";
              logger.warn(
                "Received non-Anthropic tool_choice format, defaulting to auto"
              );
            }
...
5. Your response MUST be valid JSON and include all the specified fields with their correct types`;
    ...
    if (request.stream && !shouldStreamToolCalls) {
      logger.info(
        "Using non-streaming mode for Anthropic request (tool calls executed silently)"
      );
          ...
          while (iterationCount < MAX_ITERATIONS) {
            ...
            if (
              ...
              if (remainingTools.length > 0) {
                ...
                logger.info(
                  "All forced tools have been used, removing tool_choice parameter"
                );
              }
            } else if (
              hasUsedForcedTool &&
    ...
    try {
      ...
      if (request.stream && iterationCount > 0) {
        logger.info(
          "Using streaming for final Anthropic response after tool calls"
        );
