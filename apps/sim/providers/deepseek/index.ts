 ...
 */
function createReadableStreamFromDeepseekStream(
  deepseekStream: any
): ReadableStream {
...
export const deepseekProvider: ProviderConfig = {
  ...
  executeRequest: async (
    ...
    if (!request.apiKey) {
      throw new Error("请输入Deepseek的API密钥");
    }
    ...
    try {
      ...
      if (request.stream && iterationCount > 0) {
        logger.info(
          "Using streaming for final DeepSeek response after tool calls"
        );
        ...
        const streamResponse =
          await deepseek.chat.completions.create(streamingPayload);
