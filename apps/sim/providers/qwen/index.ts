...
/**
 * Helper function to convert a Qwen stream to a standard ReadableStream
 * and collect completion metrics
 */
function createReadableStreamFromQwenStream(
  qwenStream: any,
...
export const qwenProvider: ProviderConfig = {
  ...
  version: "1.0.0",
  models: getProviderModels("qwen"),
  defaultModel: getProviderDefaultModel("qwen"),

  executeRequest: async (
    request: ProviderRequest
  ): Promise<ProviderResponse | StreamingExecution> => {
    logger.info("Preparing Qwen request", {
    ...
    // Transform tools to Qwen format if provided
    ...
    let preparedTools: ReturnType<typeof prepareToolsWithUsageControl> | null =
      null;
    ...
    if (tools?.length) {
      ...
      if (filteredTools?.length && toolChoice) {
        ...
        logger.info("Qwen request configuration:", {
    ...
    try {
      ...
      if (request.stream && (!tools || tools.length === 0)) {
        logger.info("Using streaming response for Qwen request");

        // Create a streaming request with token usage tracking
        const streamResponse = await qwen.chat.completions.create({
          ...payload,
          stream: true,
        ...
        const streamingResult = {
          stream: createReadableStreamFromQwenStream(
            streamResponse,
            (content, usage) => {
              ...
              if (streamingResult.execution.output.providerTiming) {
                streamingResult.execution.output.providerTiming.endTime =
                  streamEndTimeISO;
                ...
                if (
                  streamingResult.execution.output.providerTiming
                    .timeSegments?.[0]
                ) {
      ...
      let currentResponse = await qwen.chat.completions.create(payload);
      ...
      while (iterationCount < MAX_ITERATIONS) {
        ...
        if (
          ...
          const remainingTools = forcedTools.filter(
            (tool) => !usedForcedTools.includes(tool)
          );
        ...
        currentResponse = await qwen.chat.completions.create(nextPayload);
      ...
      if (request.stream && iterationCount > 0) {
        ...
        // This prevents Qwen API from trying to force tool usage again in the final streaming response
        ...
        const streamResponse =
          await qwen.chat.completions.create(streamingPayload);
      ...
      logger.error("Error in Qwen request:", {
        error,
        duration: totalDuration,
      });
