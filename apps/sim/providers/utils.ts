...
export function getProviderConfigFromModel(
  model: string
...
export function generateStructuredOutputInstructions(
  responseFormat: any
): string {
  ...
  if (
    ...
    (responseFormat.type === "object" && responseFormat.properties)
  ) {
    return "";
  }
  ...
  logger.info(
    `Generated structured output instructions for ${responseFormat.fields.length} fields`
  );
...
export function getApiKey(
  ...
  const isOllamaModel =
    provider === "ollama" || useOllamaStore.getState().models.includes(model);
  if (isOllamaModel) {
...
export function prepareToolsWithUsageControl(
  ...
  const forcedTools =
    providerTools?.filter((tool) => tool.usageControl === "force") || [];
  ...
  if (forcedTools.length > 0) {
    ...
    logger.info(
      "Setting tool_choice to auto - letting model decide which tools to use"
    );
  }
...
export function trackForcedToolUsage(
  ...
  if (
    isGoogleFormat &&
    originalToolChoice?.functionCallingConfig?.allowedFunctionNames
  ) {
    // For Google format
    forcedToolNames =
      originalToolChoice.functionCallingConfig.allowedFunctionNames;
  ...
  if (
    ...
    const toolNames = toolCallsResponse.map(
      (tc) => tc.function?.name || tc.name || tc.id
    );

    // Find any forced tools that were used
    const usedTools = forcedToolNames.filter((toolName) =>
    ...
    if (usedTools.length > 0) {
      ...
      const remainingTools = forcedTools.filter(
        (tool) => !updatedUsedForcedTools.includes(tool)
      );
...
export const MODELS_WITH_TEMPERATURE_SUPPORT =
  getModelsWithTemperatureSupport();
