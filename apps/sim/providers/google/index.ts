            ...
            if (closeBrace !== -1) {
              ...
              try {
                ...
                if (candidate?.finishReason === "UNEXPECTED_TOOL_CALL") {
                  logger.warn(
                    "Gemini returned UNEXPECTED_TOOL_CALL in streaming mode",
                    {
...
export const googleProvider: ProviderConfig = {
  ...
  executeRequest: async (
    ...
    if (!request.apiKey) {
      throw new Error("请输入Google Gemini的API密钥");
    }
    ...
    try {
      ...
      const { contents, tools, systemInstruction } =
      ...
      let preparedTools: ReturnType<
        typeof prepareToolsWithUsageControl
      > | null = null;
      ...
      if (request.stream && tools?.length) {
        logger.info(
          "Streaming disabled for initial request due to tools presence",
          {
      ...
      const checkForForcedToolUsage = (functionCall: {
        name: string;
        args: any;
      }) => {
        if (currentToolConfig && forcedTools.length > 0) {
          const toolCallsForTracking = [
      ...
      try {
        ...
        if (functionCall) {
          logger.info(
            `Received function call from Gemini: ${functionCall.name}`
          );
              ...
              try {
                ...
                if (request.stream) {
                  ...
                  const allForcedToolsUsed =
                    ...
                    usedForcedTools.length === forcedTools.length;
                  ...
                  if (allForcedToolsUsed && request.responseFormat) {
                    ...
                    const responseFormatSchema =
                      request.responseFormat.schema || request.responseFormat;
                    const cleanSchema =
                      cleanSchemaForGemini(responseFormatSchema);

                    if (!streamingPayload.generationConfig) {
                      streamingPayload.generationConfig = {};
                    }
                    streamingPayload.generationConfig.responseMimeType =
                      "application/json";
                    streamingPayload.generationConfig.responseSchema =
                      cleanSchema;

                    logger.info(
                      "Using structured output for final response after tool execution"
                    );
                  } else {
                  ...
                  logger.info(
                    "No function call detected, proceeding with streaming response"
                  );
                  ...
                  if (!streamingResponse.ok) {
                    ...
                    logger.error(
                      "Error in Gemini streaming follow-up request:",
                      {
                  ...
                  const stream =
                    createReadableStreamFromGeminiStream(streamingResponse);
                ...
                if (allForcedToolsUsed && request.responseFormat) {
                  ...
                  nextPayload.generationConfig.responseMimeType =
                    "application/json";
...
function convertToGeminiFormat(request: ProviderRequest): {
  ...
  const tools = request.tools?.map((tool) => {
    ...
    if (toolParameters.properties) {
      ...
      const required = toolParameters.required
