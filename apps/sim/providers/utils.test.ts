...
describe("getApiKey", () => {
  ...
  it("should throw error if no key provided in non-hosted environment", () => {
    ...
    expect(() => getApiKey("openai", "gpt-4")).toThrow(
      "请输入openai gpt-4的API密钥"
    );
    expect(() => getApiKey("anthropic", "claude-3")).toThrow(
      "请输入anthropic claude-3的API密钥"
    );
  });
  ...
  it("should require user key for non-OpenAI/Anthropic providers even in hosted environment", () => {
    ...
    expect(() => getApiKey("other-provider", "some-model")).toThrow(
      "请输入other-provider some-model的API密钥"
    );
  });
});
...
describe("Model Capabilities", () => {
  describe("supportsTemperature", () => {
    it.concurrent(
      "should return true for models that support temperature",
      () => {
    ...
    it.concurrent(
      "should return false for models that do not support temperature",
      () => {
  ...
  describe("getMaxTemperature", () => {
    it.concurrent(
      "should return 2 for models with temperature range 0-2",
      () => {
        const modelsRange02 = [
          "gpt-4o",
          "azure/gpt-4o",
    ...
    it.concurrent(
      "should return 1 for models with temperature range 0-1",
      () => {
        const modelsRange01 = [
          "claude-sonnet-4-0",
          "claude-opus-4-0",
    ...
    it.concurrent(
      "should return undefined for models that do not support temperature",
      () => {
        expect(getMaxTemperature("unsupported-model")).toBeUndefined();
        expect(getMaxTemperature("cerebras/llama-3.3-70b")).toBeUndefined();
        expect(
          getMaxTemperature("groq/meta-llama/llama-4-scout-17b-16e-instruct")
        ).toBeUndefined();
        // Reasoning models that don't support temperature
  ...
  describe("supportsToolUsageControl", () => {
    it.concurrent(
      "should return true for providers that support tool usage control",
      () => {
  ...
  describe("Model Constants", () => {
    ...
    it.concurrent(
      "should have correct providers in PROVIDERS_WITH_TOOL_USAGE_CONTROL",
      () => {
...
describe("Cost Calculation", () => {
  ...
  describe("formatCost", () => {
    ...
    it.concurrent(
      "should format costs between 1¢ and $1 with three decimal places",
      () => {
    ...
    it.concurrent(
      "should format costs between 0.1¢ and 1¢ with four decimal places",
      () => {
    ...
    it.concurrent(
      "should format very small costs with appropriate precision",
      () => {
...
describe("Provider Management", () => {
  ...
  describe("getProvider", () => {
    it.concurrent(
      "should return provider config for valid provider IDs",
      () => {
...
describe("JSON and Structured Output", () => {
  describe("extractAndParseJSON", () => {
    ...
    it.concurrent("should throw error for invalid JSON", () => {
      ...
      expect(() => extractAndParseJSON(invalidJson)).toThrow(
  ...
  describe("generateStructuredOutputInstructions", () => {
    ...
    it.concurrent(
      "should return empty string for object type with properties",
      () => {
        const objectFormat = {
          type: "object",
          properties: { key: { type: "string" } },
    ...
    it.concurrent(
      "should generate instructions for legacy fields format",
      () => {
        const fieldsFormat = {
          fields: [
...
describe("Tool Management", () => {
  describe("transformCustomTool", () => {
    ...
    it.concurrent("should throw error for invalid schema", () => {
      ...
      expect(() => transformCustomTool(invalidTool)).toThrow(
        "Invalid custom tool schema"
      );

      const noFunction = { id: "test", schema: {} };
      expect(() => transformCustomTool(noFunction)).toThrow(
        "Invalid custom tool schema"
      );
    });
  });
  ...
  describe("prepareToolsWithUsageControl", () => {
    ...
    it.concurrent("should return early for no tools", () => {
      const result = prepareToolsWithUsageControl(
        undefined,
        undefined,
        mockLogger
      );
    ...
    it.concurrent('should filter out tools with usageControl="none"', () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        "Filtered out 1 tools with usageControl='none'"
      );
    });

    it.concurrent(
      "should set toolChoice for forced tools (OpenAI format)",
      () => {
    ...
    it.concurrent(
      "should set toolChoice for forced tools (Anthropic format)",
      () => {
        const tools = [{ function: { name: "forcedTool" } }];
        const providerTools = [{ id: "forcedTool", usageControl: "force" }];
