...
export const workflowEdges = pgTable(
  ...
  (table) => ({
    ...
    sourceBlockIdx: index("workflow_edges_source_block_idx").on(
      table.sourceBlockId
    ),
    targetBlockIdx: index("workflow_edges_target_block_idx").on(
      table.targetBlockId
    ),
...
export const settings = pgTable("settings", {
  ...
  consoleExpandedByDefault: boolean("console_expanded_by_default")
    .notNull()
    .default(true),

  // Privacy settings
  telemetryEnabled: boolean("telemetry_enabled").notNull().default(true),
  telemetryNotifiedUser: boolean("telemetry_notified_user")
    .notNull()
    .default(false),
...
export const embedding = pgTable(
  "embedding",
  {
    ...
    // Vector embeddings - optimized for text-embedding-v4 with HNSW support
    embedding: vector("embedding", { dimensions: 1024 }), // For text-embedding-v4 (Qwen3-Embedding)
    embeddingModel: text("embedding_model")
      .notNull()
      .default("text-embedding-v4"),
...
export const docsEmbeddings = pgTable(
  "docs_embeddings",
  {
    ...
    // Vector embedding - optimized for text-embedding-v4 with HNSW support
    embedding: vector("embedding", { dimensions: 1024 }).notNull(), // For text-embedding-v4 (Qwen3-Embedding)
    embeddingModel: text("embedding_model")
      .notNull()
      .default("text-embedding-v4"),
