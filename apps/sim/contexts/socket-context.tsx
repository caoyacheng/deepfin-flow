...
export function SocketProvider({ children, user }: SocketProviderProps) {
  ...
  useEffect(() => {
    ...
    const initializeSocket = async () => {
      try {
        ...
        const socketUrl =
          getEnv("NEXT_PUBLIC_SOCKET_URL") || "http://localhost:3002";
        ...
        socketInstance.on("connect", () => {
          ...
          if (urlWorkflowId) {
            logger.info(
              `Joining workflow room after connection: ${urlWorkflowId}`
            );
        ...
        socketInstance.on("workflow-reverted", (data) => {
          logger.info(
            `Workflow ${data.workflowId} has been reverted to deployed state`
          );
        ...
        socketInstance.on("workflow-updated", (data) => {
          logger.info(
            `Workflow ${data.workflowId} has been updated externally - requesting sync`
          );
            ...
            try {
              ...
              if (response.ok) {
                ...
                if (workflowData?.state) {
                  logger.info(
                    "Rehydrating stores with fresh workflow state from database"
                  );
                  ...
                  Promise.all([
                    ...
                    .then(([{ useWorkflowStore }, { useSubBlockStore }]) => {
                      ...
                      Object.entries(workflowState.blocks || {}).forEach(
                        ([blockId, block]) => {
                          ...
                          Object.entries(blockState.subBlocks || {}).forEach(
                            ([subblockId, subblock]) => {
                              subblockValues[blockId][subblockId] = (
                                subblock as any
                              ).value;
                            }
                          );
                      ...
                      const newWorkflowState = {
                        ...
                        hasActiveSchedule:
                          workflowState.hasActiveSchedule || false,
                        hasActiveWebhook:
                          workflowState.hasActiveWebhook || false,
                      };
                      ...
                      logger.info(
                        "Successfully rehydrated stores from database after copilot edit"
                      );
                    })
                    .catch((error) => {
                      logger.error(
                        "Failed to import stores for copilot rehydration:",
                        error
                      );
                    });
                }
                ...
                logger.error(
                  "Failed to fetch fresh workflow state:",
                  response.statusText
                );
              }
            } catch (error) {
              logger.error(
                "Failed to rehydrate stores after copilot edit:",
                error
              );
            }
          }
        ...
        socketInstance.on("workflow-state", (workflowData) => {
          ...
          if (workflowData?.state && workflowData.id === urlWorkflowId) {
            logger.info(
              "Updating local stores with fresh workflow state from server"
            );
              ...
              Promise.all([
                ...
                .then(
                  ([
                    ...
                    // 注意: 此处不触发自动布局是因为:
                    // 1. 对于AI助手编辑: 后端已经优化了节点位置
                    // 2. 对于其他同步: 需要保留现有的节点位置
                    // 这样可以避免ID冲突和不必要的位置更新

                    logger.info(
                      "Successfully updated local stores with fresh workflow state"
                    );
                  }
                )
                .catch((error) => {
                  logger.error(
                    "Failed to import stores for workflow state update:",
                    error
                  );
                });
            } catch (error) {
              logger.error(
                "Failed to update local stores with workflow state:",
                error
              );
            }
          }
  ...
  useEffect(() => {
    ...
    if (currentWorkflowId) {
      logger.info(
        `Leaving current workflow ${currentWorkflowId} before joining ${urlWorkflowId}`
      );
  ...
  const joinWorkflow = useCallback(
    (workflowId: string) => {
      ...
      if (currentWorkflowId) {
        logger.info(
          `Leaving current workflow ${currentWorkflowId} before joining ${workflowId}`
        );
        socket.emit("leave-workflow");
      }
