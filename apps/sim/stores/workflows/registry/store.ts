  ...
  devtools(
    (set, get) => ({
          ...
          deploymentStatuses: {
            ...
            [workflowId as string]: {
              ...
              needsRedeployment: isDeployed
                ? false
                : ((state.deploymentStatuses?.[workflowId as string] as any)
                    ?.needsRedeployment ?? false),
            },
          },
        }));
          ...
          workflowState = {
            ...
            history: {
              ...
              present: {
                ...
                timestamp: Date.now(),
                action:
                  "Empty initial state - server should provide start block",
                subblockValues: {},
              },
              future: [],
            },
      ...
      createWorkflow: async (options = {}) => {
        ...
        logger.info(
          `Creating new workflow in workspace: ${workspaceId || "none"}`
        );
        ...
        try {
          ...
          logger.info(
            `Successfully created workflow ${serverWorkflowId} on server`
          );
          ...
          if (options.marketplaceId && options.marketplaceState) {
            ...
            logger.info(
              `Created workflow from marketplace: ${options.marketplaceId}`
            );
          } else {
          ...
          if (options.marketplaceId && options.marketplaceState?.blocks) {
            useSubBlockStore
              ...
              .initializeFromWorkflow(
                serverWorkflowId,
                options.marketplaceState.blocks
              );
          }
      ...
      createMarketplaceWorkflow: async (
        ...
        const persistWorkflow = async () => {
          try {
            ...
            if (!response.ok) {
              throw new Error(
                `Failed to persist workflow: ${response.statusText}`
              );
            }

            logger.info(
              `Successfully persisted marketplace workflow ${id} to database`
            );
          } catch (error) {
            logger.error(
              `Failed to persist marketplace workflow ${id}:`,
              error
            );
          }
        };
        ...
        logger.info(
          `Created marketplace workflow ${id} imported from ${marketplaceId}`
        );
      ...
      duplicateWorkflow: async (sourceId: string) => {
        ...
        try {
          ...
          if (!response.ok) {
            throw new Error(
              `Failed to duplicate workflow: ${response.statusText}`
            );
          }
        ...
        if (sourceId === get().activeWorkflowId) {
          const sourceSubblockValues =
            useSubBlockStore.getState().workflowValues[sourceId] || {};
          useSubBlockStore.setState((state) => ({
            workflowValues: {
              ...state.workflowValues,
      ...
      removeWorkflow: async (id: string) => {
        ...
        set((state) => {
          ...
          fetch(API_ENDPOINTS.SCHEDULE, {
            ...
            logger.error(
              `Error cancelling schedule for deleted workflow ${id}:`,
              error
            );
          });
