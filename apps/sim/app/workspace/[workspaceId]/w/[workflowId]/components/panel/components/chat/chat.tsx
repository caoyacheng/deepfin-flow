...
export function Chat({ panelWidth, chatMessage, setChatMessage }: ChatProps) {
  ...
  const outputEntries = useMemo(() => {
    ...
    return entries.filter(
      (entry) => entry.workflowId === activeWorkflowId && entry.output
    );
  ...
  const handleSendMessage = useCallback(async () => {
    ...
    if (
      ...
      (promptHistory.length === 0 ||
        promptHistory[promptHistory.length - 1] !== sentMessage)
    ) {
    ...
    try {
      ...
      addMessage({
        content:
          sentMessage ||
          (chatFiles.length > 0 ? `上传了${chatFiles.length}个文件` : ""),
              ...
              try {
                ...
                if (event === "final" && data) {
                  ...
                  const nonStreamingLogs =
                    result.logs?.filter(
                      (log) => !messageIdMap.has(log.blockId)
                    ) || [];

                  if (nonStreamingLogs.length > 0) {
                    const outputsToRender = selectedOutputs.filter(
                      (outputId) => {
                        const blockIdForOutput =
                          extractBlockIdFromOutputId(outputId);
                        return nonStreamingLogs.some(
                          (log) => log.blockId === blockIdForOutput
                        );
                      }
                    );
                    ...
                    for (const outputId of outputsToRender) {
                      const blockIdForOutput =
                        extractBlockIdFromOutputId(outputId);
                      const path = extractPathFromOutputId(
                        outputId,
                        blockIdForOutput
                      );
                      const log = nonStreamingLogs.find(
                        (l) => l.blockId === blockIdForOutput
                      );
      ...
      if (selectedOutputs?.length > 0) {
        for (const outputId of selectedOutputs) {
          ...
          const log = result.logs?.find(
            (l: BlockLog) => l.blockId === blockIdForOutput
          );
        ...
        >
          ...
          <div className="flex gap-2">
            <Input
              ...
              placeholder={isDragOver ? "拖拽文件到这里" : "输入消息..."}
