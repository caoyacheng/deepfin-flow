...
export function FieldFormat({
  ...
  const [storeValue, setStoreValue] = useSubBlockValue<Field[]>(
    blockId,
    subBlockId
  );
  ...
  const updateField = (id: string, field: keyof Field, value: any) => {
    ...
    setStoreValue(
      fields.map((f: Field) => (f.id === id ? { ...f, [field]: value } : f))
    );
  };
      ...
      >
        <div className="flex items-center">
          ...
          {field.name && showType && (
            <Badge
              variant="outline"
              className="ml-2 h-5 bg-muted py-0 font-normal text-xs"
            >
              {field.type}
            </Badge>
          )}
                    ...
                    <div className="space-y-1.5">
                      ...
                      <div className="relative">
                        <Input
                          ...
                          value={field.value || ""}
                          onChange={(e) =>
                            handleValueInputChange(field.id, e.target.value)
                          }
                          onKeyDown={(e) => {
                            if (e.key === "Escape") {
                        ...
                        <TagDropdown
                          visible={
                            tagDropdownStates[field.id]?.visible || false
                          }
                          onSelect={(newValue) =>
                            handleTagSelect(field.id, newValue)
                          }
                          blockId={blockId}
                          activeSourceBlockId={null}
                          inputValue={field.value || ""}
                          cursorPosition={
                            tagDropdownStates[field.id]?.cursorPosition || 0
                          }
