...
export function Panel() {
  ...
  const ensureCopilotDataLoaded = useCallback(
    async (forceRefresh = false) => {
      try {
        ...
        console.error("加载 Copilot 数据失败:", error);
      }
    },
    [
      activeWorkflowId,
  ...
  const handleHistoryDropdownOpen = useCallback(
    async (open: boolean) => {
      ...
      if (open && isSendingMessage) {
        console.log("正在流式响应中打开聊天记录 - 仅显示缓存数据");
      }
    },
    ...
    <div className="px-1 py-1">
      ...
      <div className="mt-1 flex flex-col gap-1">
        {[1, 2, 3].map((i) => (
          <div
            key={i}
            className="mx-1 flex h-8 items-center rounded-lg px-2 py-1.5"
          >
  ...
  const handleTabClick = async (
    tab: "chat" | "console" | "variables" | "copilot"
  ) => {
    setActiveTab(tab);
    if (!isOpen) {
      togglePanel();
  ...
  useEffect(() => {
    ...
    if (
      activeWorkflowId &&
      activeWorkflowId !== lastLoadedWorkflowRef.current
    ) {
      ...
      if (copilotWorkflowId !== activeWorkflowId || !copilotWorkflowId) {
        ensureCopilotDataLoaded().catch((error) => {
          console.error("工作流切换时自动加载 Copilot 数据失败:", error);
        });
      }
    }
              ...
              {activeTab === "console" && (
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button
                      onClick={() =>
                        activeWorkflowId && exportConsoleCSV(activeWorkflowId)
                      }
                  ...
                  <TooltipContent side="bottom">导出控制台数据</TooltipContent>
                </Tooltip>
              )}
              {activeTab === "chat" && (
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button
                      onClick={() =>
                        activeWorkflowId && exportChatCSV(activeWorkflowId)
                      }
                      className="font-medium text-md leading-normal transition-[filter] hover:brightness-75 focus:outline-none focus-visible:outline-none active:outline-none dark:hover:brightness-125"
                      style={{ color: "var(--base-muted-foreground)" }}
                    >
                  ...
                  <TooltipContent side="bottom">导出对话数据</TooltipContent>
                </Tooltip>
              )}
              {activeTab === "copilot" && (
                <>
