...
export function DocumentTagEntry({
  ...
  const [storeValue, setStoreValue] = useSubBlockValue<string>(
    blockId,
    subBlock.id
  );
  ...
  const { tagDefinitions, isLoading } =
  ...
  const [typeDropdownStates, setTypeDropdownStates] = useState<
  ...
  const usedTagNames = new Set(
    rows
      .map((row) => row.cells.tagName?.toLowerCase())
  ...
  const newTagsBeingCreated = rows.filter(
    (row) =>
      ...
      !tagDefinitions.some(
        (def) =>
          def.displayName.toLowerCase() === row.cells.tagName.toLowerCase()
      )
  ).length;
  const canAddMoreTags =
    tagDefinitions.length + newTagsBeingCreated < MAX_TAG_SLOTS;
  ...
  const handlePreFillTags = () => {
    ...
    const jsonString =
      existingTagRows.length > 0 ? JSON.stringify(existingTagRows) : "";
  ...
  const handleTagDropdownSelection = (
    rowIndex: number,
    column: string,
    value: string
  ) => {
  ...
  const handleAddRow = () => {
    ...
    const newData = [
      ...currentData,
      { id: newRowId, tagName: "", fieldType: "text", value: "" },
    ];
  ...
  const handleDeleteRow = (rowIndex: number) => {
    ...
    const jsonString =
      tableDataForStorage.length > 0 ? JSON.stringify(tableDataForStorage) : "";
    setStoreValue(jsonString);
  };
  ...
  const renderHeader = () => (
    <thead>
      <tr className="border-b">
        <th className="w-2/5 border-r px-4 py-2 text-center font-medium text-sm">
          Tag Name
        </th>
        <th className="w-1/5 border-r px-4 py-2 text-center font-medium text-sm">
          Type
        </th>
      ...
      <td className="relative border-r p-1">
        <div className="relative w-full">
          <Input
            value={cellValue}
            onChange={(e) =>
              handleCellChange(rowIndex, "tagName", e.target.value)
            }
            onFocus={handleFocus}
            onBlur={handleBlur}
      ...
      {activeTagDropdown?.element && (
        <TagDropdown
          ...
          onSelect={(newValue) => {
            ...
            handleTagDropdownSelection(
              activeTagDropdown.rowIndex,
              "value",
              newValue
            );
            setActiveTagDropdown(null);
          ...
          onClose={() => {
            setActiveTagDropdown((prev) =>
              prev ? { ...prev, showTags: false } : null
            );
          }}
      ...
      {!isPreview && !disabled && (
        <div className="mt-3 flex items-center justify-between">
          ...
          <div className="text-muted-foreground text-xs">
            {tagDefinitions.length + newTagsBeingCreated} of {MAX_TAG_SLOTS}
            标签槽位已使用
          </div>
        </div>
      )}
