...
export function Code({
  ...
  const aiPromptPlaceholder = useMemo(() => {
    switch (generationType) {
      case "json-schema":
        return "描述要生成的 JSON 模式";
      case "json-object":
        return "描述要生成的 JSON 对象";
      default:
        return "描述要生成的 JavaScript 代码";
    }
  }, [generationType]);
  ...
  useEffect(() => {
    ...
    const calculateVisualLines = () => {
      ...
      setLineCount(
        newVisualLineHeights.reduce((sum, height) => sum + height, 0)
      );
    };
  ...
  const renderLineNumbers = () => {
    ...
    visualLineHeights.forEach((height, index) => {
      numbers.push(
        <div
          key={`${lineNumber}-0`}
          className={cn("text-muted-foreground text-xs leading-[21px]")}
        >
          {lineNumber}
        </div>
      );
      for (let i = 1; i < height; i++) {
        numbers.push(
          <div
            key={`${lineNumber}-${i}`}
            className={cn(
              "invisible text-muted-foreground text-xs leading-[21px]"
            )}
          >
            {lineNumber}
          </div>
            ...
            onValueChange={(newCode) => {
              if (!isCollapsed && !isAiStreaming && !isPreview && !disabled) {
                ...
                if (textarea) {
                  ...
                  setSearchTerm(
                    envVarTrigger.show ? envVarTrigger.searchTerm : ""
                  );
                }
              }
            }}
