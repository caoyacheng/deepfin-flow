...
export function ToolInput({
  ...
  const supportsToolControl = provider
    ? supportsToolUsageControl(provider)
    : false;
  ...
  const handleSelectTool = (toolBlock: (typeof toolBlocks)[0]) => {
    ...
    const operationOptions = hasOperations
      ? getOperationOptions(toolBlock.type)
      : [];
    const defaultOperation =
      operationOptions.length > 0 ? operationOptions[0].id : undefined;
  ...
  const handleOperationChange = (toolIndex: number, operation: string) => {
    ...
    const oldParamIds = new Set(
      oldToolParams?.userInputParameters.map((p) => p.id) || []
    );
    const newParamIds = new Set(
      toolParams.userInputParameters.map((p) => p.id)
    );
  ...
  const evaluateParameterCondition = (
    param: any,
    tool: StoredTool
  ): boolean => {
  ...
  const getCredentialForChannelSelector = (paramId: string): string => {
    ...
    const botToken =
      currentTool.params.botToken ||
      (subBlockStore.getValue(blockId, "botToken") as string);
    const oauthCredential =
      currentTool.params.credential ||
    ...
    for (const key of credentialKeys) {
      const value =
        currentTool.params[key] ||
        (subBlockStore.getValue(blockId, key) as string);
      if (value?.trim()) {
        return value;
      }
    }
  ...
  const renderParameterInput = (
    ...
    const uniqueBlockId =
      toolIndex !== undefined ? `${blockId}-tool-${toolIndex}` : blockId;
      ...
      case "oauth-input":
        return (
          <ToolCredentialSelector
            ...
            onChange={onChange}
            provider={
              (uiComponent.provider || uiComponent.serviceId) as OAuthProvider
            }
  ...
  return (
    <div className="w-full">
      {selectedTools.length === 0 ? (
                        ...
                        {toolBlocks
                          .filter(
                            (block) =>
                              ...
                              block.name !== "YouTube" &&
                              block.name !== "X" &&
                              block.name !== "Wikipedia" &&
                              block.name !== "WhatsApp" &&
                              block.name !== "Wealthbox" &&
                              block.name !== "Typeform" &&
                              block.name !== "Twilio SMS" &&
                              block.name !== "Telegram" &&
                              block.name !== "Tavily" &&
                              block.name !== "Slack" &&
                              block.name !== "Stagehand Agent" &&
                              block.name !== "Stagehand Extract" &&
                              block.name !== "Sharepoint" &&
                              block.name !== "Serper" &&
                              block.name !== "Reddit" &&
                              block.name !== "Perplexity" &&
                              block.name !== "OneDrive" &&
                              block.name !== "Outlook" &&
                              block.name !== "Notion" &&
                              block.name !== "Microsoft Teams" &&
                              block.name !== "Microsoft Planner" &&
                              block.name !== "Microsoft Excel" &&
                              block.name !== "Linear" &&
                              block.name !== "Hunter io" &&
                              block.name !== "Hugging Face" &&
                              block.name !== "Google Sheets" &&
                              block.name !== "Google Search" &&
                              block.name !== "Google Drive" &&
                              block.name !== "Google Calendar" &&
                              block.name !== "Google Docs" &&
                              block.name !== "GitHub" &&
                              block.name !== "Discord" &&
                              block.name !== "Confluence" &&
                              block.name !== "Clay" &&
                              block.name !== "Airtable"
                          )
                          .map((block) => (
        ...
        <div className="flex min-h-[2.5rem] w-full flex-wrap gap-2 rounded-md border border-input bg-transparent px-3 py-2 text-sm ring-offset-background">
          {selectedTools.map((tool, toolIndex) => {
            ...
            const customToolParams =
              isCustomTool && tool.schema
                ? Object.entries(
                  ...
                  ).map(([paramId, param]: [string, any]) => ({
                    id: paramId,
                    type: param.type || "string",
                    description: param.description || "",
                    visibility:
                      (tool.schema.function.parameters.required?.includes(
                        paramId
                      )
                        ? "user-or-llm"
                        : "user-only") as
            ...
            const requiresOAuth =
              !isCustomTool && toolRequiresOAuth(currentToolId);
            const oauthConfig = !isCustomTool
                        ...
                        <Tooltip>
                          ...
                          >
                            <p className="text-xs">
                              {tool.usageControl === "auto" && (
                                <span>
                                  <span className="font-medium">Auto:</span> Let
                              ...
                              {tool.usageControl === "force" && (
                                <span>
                                  ...
                                  Always use this tool in the response
                                </span>
                              )}
                              {tool.usageControl === "none" && (
                                <span>
                  ...
                  {!isCustomTool && tool.isExpanded && (
                    <div className="space-y-3 overflow-visible p-3">
                          ...
                          <div className="relative min-w-0 space-y-1.5">
                            ...
                            <div className="w-full min-w-0">
                              <Select
                                ...
                                onValueChange={(value) =>
                                  handleOperationChange(toolIndex, value)
                                }
                              >
                      ...
                      {(() => {
                        ...
                        const groupedParams: {
                          [key: string]: ToolParameterConfig[];
                        } = {};
                        ...
                        Object.entries(groupedParams).forEach(
                          ([groupTitle, params]) => {
      ...
      <CustomToolModal
        ...
        onSave={
          editingToolIndex !== null ? handleSaveCustomTool : handleAddCustomTool
        }
        onDelete={handleDeleteTool}
        initialValues={
          editingToolIndex !== null &&
          selectedTools[editingToolIndex]?.type === "custom-tool"
            ? {
                id: customTools.find(
                  (tool) =>
                    tool.schema.function.name ===
                    selectedTools[editingToolIndex].id.replace("custom_", "")
                )?.id,
