...
export function WorkflowBlock({ id, data }: NodeProps<WorkflowBlockProps>) {
  ...
  const diffStatus =
    currentWorkflow.isDiffMode && currentBlock
      ? (currentBlock as any).is_diff
      : undefined;

  // Get field-level diff information
  const fieldDiff =
    currentWorkflow.isDiffMode && currentBlock
      ? (currentBlock as any).field_diffs
      : undefined;
  ...
  const isDeletedBlock =
    !isShowingDiff && diffAnalysis?.deleted_blocks?.includes(id);
  ...
  useEffect(() => {
    ...
    if (diffAnalysis && !isShowingDiff) {
      console.log(
        `[WorkflowBlock ${id}] Diff analysis available in original workflow:`,
        {
    ...
    if (isDeletedBlock) {
      console.log(
        `[WorkflowBlock ${id}] Block marked for deletion in original workflow`
      );
    }
  }, [
    currentWorkflow.isDiffMode,
  ...
  const blockHeight = useWorkflowStore(
    (state) => state.blocks[id]?.height ?? 0
  );
  ...
  const hasWebhookProvider = useSubBlockStore(
    (state) =>
      state.workflowValues[activeWorkflowId || ""]?.[id]?.webhookProvider
  );
  ...
  const blockAdvancedMode = useWorkflowStore(
    (state) => state.blocks[id]?.advancedMode ?? false
  );
  const blockTriggerMode = useWorkflowStore(
    (state) => state.blocks[id]?.triggerMode ?? false
  );
  ...
  const updateBlockHeight = useWorkflowStore(
    (state) => state.updateBlockHeight
  );

  // Execution store
  const isActiveBlock = useExecutionStore((state) =>
    state.activeBlockIds.has(id)
  );
  ...
  const disableSchedule = async (scheduleId: string) => {
    try {
      ...
      if (response.ok) {
        ...
        console.error("禁用执行计划失败");
      }
    } catch (error) {
      console.error("禁用执行计划失败:", error);
    }
  };
  ...
  function groupSubBlocks(subBlocks: SubBlockConfig[], blockId: string) {
    ...
    if (data.isPreview && data.subBlockValues) {
      ...
      const activeWorkflowId =
        useWorkflowRegistry.getState().activeWorkflowId || undefined;
      const mergedState = mergeSubblockState(blocks, activeWorkflowId, blockId)[
        blockId
      ];
    ...
    const visibleSubBlocks = subBlocks.filter((block) => {
      ...
      const actualCondition =
        typeof block.condition === "function"
          ? block.condition()
          : block.condition;
        ...
        (Array.isArray(actualCondition.and.value)
          ? andFieldValue != null &&
            (actualCondition.and.not
              ? !actualCondition.and.value.includes(
                  andFieldValue as string | number | boolean
                )
              : actualCondition.and.value.includes(
                  andFieldValue as string | number | boolean
                ))
  ...
  const showWebhookIndicator =
    (isStarterBlock || isWebhookTriggerBlock) && blockWebhookStatus;
          ...
          <div className="flex flex-shrink-0 items-center gap-2">
            ...
            {shouldShowScheduleBadge && (
              <Tooltip>
                ...
                <TooltipContent side="top" className="max-w-[300px] p-4">
                  {scheduleInfo?.isDisabled ? (
                    <p className="text-sm">
                      This schedule is currently disabled. Click the badge to
                      reactivate it.
                    </p>
                  ) : (
                    <p className="text-sm">点击禁用执行计划</p>
                  )}
                </TooltipContent>
              </Tooltip>
            )}
            ...
            {config.docsLink ? (
              <Tooltip>
                ...
                <TooltipContent side="top">查看文档</TooltipContent>
              </Tooltip>
            ) : (
                ...
                <Tooltip>
                  ...
                  <TooltipContent side="top" className="max-w-[300px] p-4">
                    <div className="space-y-3">
                      <div>
                        <p className="mb-1 font-medium text-sm">描述</p>
                        <p className="text-muted-foreground text-sm">
                          {config.longDescription}
                        </p>
                      </div>
                      {config.outputs && (
                        <div>
                          <p className="mb-1 font-medium text-sm">输出</p>
                          <div className="text-sm">
                            {Object.entries(config.outputs).map(
                              ([key, value]) => (
            ...
            {config.category !== "triggers" &&
              ...
              !blockTriggerMode && (
                <Handle
                  type="source"
                  position={
                    horizontalHandles ? Position.Right : Position.Bottom
                  }
                  id="error"
                  className={cn(
                    horizontalHandles ? "!w-[7px] !h-5" : "!w-5 !h-[7px]",
