...
export function SearchModal({
  ...
  const [localTemplates, setLocalTemplates] =
    useState<TemplateData[]>(templates);
  ...
  const { navigate, getCurrentItem, scrollRefs } = useSearchNavigation(
    navigationSections,
    open
  );
      ...
      setLocalTemplates((prevTemplates) =>
        prevTemplates.map((template) =>
          template.id === templateId
            ? { ...template, isStarred, stars: newStarCount }
            : template
        )
      );
    },
  ...
  const isItemSelected = useCallback(
    (sectionId: string, itemIndex: number) => {
      ...
      return (
        ...
        current.position.itemIndex === itemIndex
      );
    },
    [getCurrentItem]
  );
                      ...
                      : filteredTemplates.map((template, index) => (
                          <div
                            ...
                            className={`w-80 flex-shrink-0 rounded-lg transition-all duration-200 ${
                              isItemSelected("templates", index)
                                ? "opacity-75"
                                : "opacity-100"
                            }`}
                          >
                    ...
                    <div className="mb-6">
                      ...
                      <div className="space-y-1 px-6">
                        {filteredWorkflows.map((workflow, workflowIndex) => {
                          const globalIndex =
                            filteredWorkspaces.length + workflowIndex;
                          return (
                            <button
                              key={workflow.id}
                              onClick={() =>
                        ...
                        {filteredPages.map((page, pageIndex) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              {page.shortcut && (
                                <KeyboardShortcut shortcut={page.shortcut} />
                              )}
                            </button>
                          );
                        })}
