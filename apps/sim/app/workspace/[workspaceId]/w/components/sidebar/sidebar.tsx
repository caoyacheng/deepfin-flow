...
export function Sidebar() {
  ...
  const [isWorkspaceSelectorVisible, setIsWorkspaceSelectorVisible] =
    useState(false);
      ...
      if (data.workspaces && Array.isArray(data.workspaces)) {
        ...
        if (currentWorkspaceId) {
          ...
          if (matchingWorkspace) {
            ...
            logger.warn(
              `Workspace ${currentWorkspaceId} not found in user's workspaces`
            );
            ...
            if (fetchedWorkspaces.length > 0) {
              ...
              logger.info(
                `Redirecting to fallback workspace: ${fallbackWorkspace.id}`
              );
  ...
  const switchWorkspace = useCallback(
    async (workspace: Workspace) => {
      ...
      try {
        ...
        logger.info(
          `Switched to workspace: ${workspace.name} (${workspace.id})`
        );
      } catch (error) {
    ...
    async (workspaceToDelete: Workspace) => {
      ...
      try {
        ...
        const response = await fetch(
          `/api/workspaces/${workspaceToDelete.id}`,
          {
            method: "DELETE",
          }
        );
        ...
        if (isDeletingCurrentWorkspace) {
          ...
          logger.info(
            "Deleting non-current workspace - using simple list refresh"
          );
          await refreshWorkspaceList();
        }
      } catch (error) {
  ...
  const handleLeaveWorkspace = useCallback(
    async (workspaceToLeave: Workspace) => {
      ...
      try {
        ...
        const response = await fetch(
          `/api/workspaces/members/${sessionData?.user?.id}`,
          {
            method: "DELETE",
            headers: {
              "Content-Type": "application/json",
        ...
        if (isLeavingCurrentWorkspace) {
          ...
          logger.info(
            "Leaving non-current workspace - using simple list refresh"
          );
          await refreshWorkspaceList();
        }
      } catch (error) {
  ...
  const fetchTemplates = useCallback(async () => {
    ...
    try {
      ...
      logger.info(
        `Templates loaded successfully: ${fetchedTemplates.length} templates`
      );
    } catch (error) {
    ...
    if (workspaceId) {
      ...
      isWorkspaceValid(workspaceId).then((valid) => {
        if (valid) {
          ...
          logger.warn(
            `Workspace ${workspaceId} no longer exists, triggering workspace refresh`
          );
  ...
  useEffect(() => {
    const handleDragStart = (e: DragEvent) => {
      ...
      if (
        target.draggable ||
        target.closest('[data-workflow-id], [draggable="true"]')
      ) {
        setIsDragging(true);
      }
    };
    ...
    >
      <span className="flex items-center justify-center gap-[1px] pt-[1px]">
        {parts.map((part, index) => (
          <span
            key={index}
            className={cn(isSymbol(part) ? "text-[17px]" : "text-xs")}
          >
            {part}
          </span>
        ))}
