      ...
      if (response.ok) {
        ...
        const updatedServices = serviceDefinitions.map((service) => {
          ...
          const connectionWithScopes = connections.find((conn: any) => {
            ...
            if (
              !conn.baseProvider ||
              !service.providerId.startsWith(conn.baseProvider)
            ) {
              return false;
            }
            ...
            if (conn.scopes && service.scopes) {
              return service.scopes.every((scope) =>
                conn.scopes.includes(scope)
              );
            }
          ...
          prev.map((svc) => {
            if (svc.id === service.id) {
              return {
                ...svc,
                accounts:
                  svc.accounts?.filter((acc) => acc.id !== accountId) || [],
    ...
    <div className="space-y-6 p-6">
      <div>
        <div className="mb-1 flex items-center justify-between">
          <h3 className="font-medium text-lg">鉴权</h3>
        ...
        <div className="mb-6 flex items-start gap-3 rounded-md border border-primary/20 bg-primary/5 p-5 text-sm shadow-sm">
          ...
          <div className="flex flex-1 flex-col">
            <p className="text-muted-foreground">
              <span className="font-medium text-primary">Action Required:</span>{" "}
              Please connect your account to enable the requested features. The
              required service is highlighted below.
            </p>
            <Button
              variant="outline"
              size="sm"
                                  ...
                                  {service.accounts.map((account) => (
                                    ...
                                    >
                                      ...
                                      <Button
                                        ...
                                        size="sm"
                                        onClick={() =>
                                          handleDisconnect(service, account.id)
                                        }
                                        disabled={
                                          isConnecting ===
