...
export function Toolbar({
  ...
  const { regularBlocks, specialBlocks, tools, triggers } = useMemo(() => {
    ...
    const regularBlockConfigs = filteredBlocks.filter(
      (block) => block.category === "blocks"
    );
    const toolConfigs = filteredBlocks.filter(
      (block) => block.category === "tools"
    );
    const triggerConfigs = filteredBlocks.filter(
      (block) => block.category === "triggers"
    );
    ...
    if (
      !searchQuery.trim() ||
      "loop".toLowerCase().includes(searchQuery.toLowerCase())
    ) {
    ...
    if (
      !searchQuery.trim() ||
      "parallel".toLowerCase().includes(searchQuery.toLowerCase())
    ) {
      specialBlockItems.push({
        name: "Parallel",
        type: "parallel",
