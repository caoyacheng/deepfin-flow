...
export function KnowledgeBase({
  ...
  const knowledgeBaseName =
    knowledgeBase?.name || passedKnowledgeBaseName || "Knowledge Base";
  ...
  const checkForDeadProcesses = async () => {
    ...
    const staleDocuments = documents.filter((doc) => {
      ...
      const processingDuration =
        now.getTime() - new Date(doc.processingStartedAt).getTime();
    ...
    const markFailedPromises = staleDocuments.map(async (doc) => {
      try {
        const response = await fetch(
          `/api/knowledge/${id}/documents/${doc.id}`,
          {
            method: "PUT",
            headers: {
        ...
        if (!response.ok) {
          ...
          logger.error(
            `Failed to mark document ${doc.id} as failed: ${errorData.error}`
          );
          return;
        }
  ...
  const handleDocumentClick = (docId: string) => {
    ...
    router.push(
      `/workspace/${workspaceId}/knowledge/${id}/${docId}?${urlParams.toString()}`
    );
  };
  ...
  const handleBulkEnable = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        result.data.updatedDocuments.forEach(
          (updatedDoc: { id: string; enabled: boolean }) => {
            updateDocument(updatedDoc.id, { enabled: updatedDoc.enabled });
          }
        );

        logger.info(
          `Successfully enabled ${result.data.successCount} documents`
        );
      }
  ...
  const handleBulkDisable = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        logger.info(
          `Successfully disabled ${result.data.successCount} documents`
        );
      }
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      if (result.success) {
        logger.info(
          `Successfully deleted ${result.data.successCount} documents`
        );
      }
  ...
  const selectedDocumentsList = documents.filter((doc) =>
    selectedDocuments.has(doc.id)
  );
  const enabledCount = selectedDocumentsList.filter(
    (doc) => doc.enabled
  ).length;
  const disabledCount = selectedDocumentsList.filter(
    (doc) => !doc.enabled
  ).length;
  ...
  if (
    (isLoadingKnowledgeBase || isLoadingDocuments) &&
    !knowledgeBase &&
    documents.length === 0
  ) {
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <Tooltip>
                      ...
                      {userPermissions.canEdit !== true && (
                        <TooltipContent>
                          Write permission required to add documents
                        </TooltipContent>
                      )}
                    </Tooltip>
                  </div>
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              您确定要删除“{knowledgeBaseName}
              ”吗？此操作将永久删除该知识库及其包含的全部 {totalItems}{" "}
              个文档，此操作无法撤销。
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
