...
import {
  ...
  useTagDefinitions,
} from "@/hooks/use-tag-definitions";
...
export function DocumentTagEntry({
  ...
  const { getNextAvailableSlot: getServerNextSlot } =
    useNextAvailableSlot(knowledgeBaseId);

  // Use the document-level hook since we have documentId
  const { saveTagDefinitions } = documentTagHook;
  const {
    tagDefinitions: kbTagDefinitions,
    fetchTagDefinitions: refreshTagDefinitions,
  } = kbTagHook;
    ...
    try {
      ...
      if (editingTagIndex !== null) {
        ...
        if (existingDefinition) {
          ...
          if (!serverSlot) {
            throw new Error(
              `No available slots for new tag of type '${editForm.fieldType}'`
            );
          }
      ...
      if (editingTagIndex !== null) {
        ...
        const currentDefinition = kbTagDefinitions.find(
          (def) =>
            ...
            currentTag.displayName.toLowerCase()
        );
        ...
        if (currentDefinition) {
          ...
          if (saveTagDefinitions) {
            ...
            throw new Error(
              "Cannot save tag definitions without a document ID"
            );
          }
            ...
            kbTagDefinitions.length >= MAX_TAG_SLOTS && (
              <div className="rounded-md border border-amber-200 bg-amber-50 p-3">
                ...
                <p className="mt-1 text-amber-700 text-xs">
                  You can still use existing tag definitions from the dropdown,
