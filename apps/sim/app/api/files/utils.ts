...
/**
 * Check if a path points to cloud storage (S3, Blob, OSS, or generic cloud)
 */
export function isCloudPath(path: string): boolean {
  return isS3Path(path) || isBlobPath(path) || isOSSPath(path);
}
...
/**
 * Check if a path points to OSS storage
 */
export function isOSSPath(path: string): boolean {
  return path.includes("/api/files/serve/oss/");
}

/**
 * Extract OSS key from a path
 */
export function extractOSSKey(path: string): string {
  return extractStorageKey(path, "oss");
}
