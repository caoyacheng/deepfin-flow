...
export async function GET() {
  ...
  await ensureWorkflowsHaveWorkspace(
    session.user.id,
    userWorkspaces[0].workspace.id
  );
...
async function createWorkspace(userId: string, name: string) {
  ...
  try {
    ...
    logger.error(
      `Failed to create workspace ${workspaceId} with initial workflow:`,
      error
    );
    throw error;
  }
...
async function ensureWorkflowsHaveWorkspace(
  userId: string,
  ...
  if (orphanedWorkflows.length > 0) {
    ...
    logger.info(
      `Fixed ${orphanedWorkflows.length} orphaned workflows for user ${userId}`
    );
  }
}
