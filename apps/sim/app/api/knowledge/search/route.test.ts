...
vi.mock("drizzle-orm", () => ({
  ...
  inArray: vi
    .fn()
    .mockImplementation((field, values) => ({ inArray: [field, values] })),
  ...
  sql: vi.fn().mockImplementation((strings, ...values) => ({
    ...
    as: vi
      .fn()
      .mockReturnValue({ sql: strings, values, alias: "mocked_alias" }),
  })),
}));
...
describe("Knowledge Search API Route", () => {
  ...
  describe("POST /api/knowledge/search", () => {
    ...
    it("should handle workflow-based authentication", async () => {
      ...
      expect(response.status).toBe(200);
      expect(data.success).toBe(true);
      expect(mockGetUserId).toHaveBeenCalledWith(
        expect.any(String),
        "workflow-123"
      );
    });
    ...
    it.concurrent(
      "should return not found for workflow that does not exist",
      async () => {
        const workflowData = {
          ...validSearchData,
    ...
    it("should return not found for some missing knowledge bases", async () => {
      ...
      expect(data.error).toBe(
        "Knowledge bases not found or access denied: kb-missing"
      );
    });
    ...
    it.concurrent("should handle database errors during search", async () => {
      ...
      mockHandleVectorOnlySearch.mockRejectedValueOnce(
    ...
    describe("Cost tracking", () => {
      it.concurrent(
        "should include cost information in successful search response",
        async () => {
          mockGetUserId.mockResolvedValue("user-123");
      ...
      it("should call cost calculation functions with correct parameters", async () => {
        ...
        expect(estimateTokenCount).toHaveBeenCalledWith(
          "test search query",
          "openai"
        );
      ...
      expect(data.details).toEqual(
        expect.arrayContaining([
          expect.objectContaining({
            message: "请提供搜索查询或标签过滤条件以搜索您的知识库",
          }),
        ])
      );
    });
