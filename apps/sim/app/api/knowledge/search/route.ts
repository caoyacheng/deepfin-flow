...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    try {
        ...
        try {
          ...
          Object.entries(validatedData.filters).forEach(([key, value]) => {
            if (value) {
              ...
              logger.debug(
                `[${requestId}] Mapped filter: "${key}" -> "${tagSlot}" = "${value}"`
              );
            }
          });
      ...
      const hasQuery =
        validatedData.query && validatedData.query.trim().length > 0;
      const queryEmbedding = hasQuery
        ? await generateSearchEmbedding(validatedData.query!)
        : null;

      // Check if any requested knowledge bases were not accessible
      const inaccessibleKbIds = knowledgeBaseIds.filter(
        (id) => !accessibleKbIds.includes(id)
      );
      ...
      if (!hasQuery && hasFilters) {
        ...
        const strategy = getQueryStrategy(
          accessibleKbIds.length,
          validatedData.topK
        );
      ...
      if (hasQuery) {
        try {
          ...
          logger.warn(
            `[${requestId}] Failed to calculate cost for search query`,
            {
