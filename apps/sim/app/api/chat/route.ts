...
export async function GET(request: NextRequest) {
  try {
    ...
    return createErrorResponse(
      error.message || "Failed to fetch chat deployments",
      500
    );
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      const { hasAccess, workflow: workflowRecord } =
        await checkWorkflowAccessForChatCreation(workflowId, session.user.id);
      ...
      const mergedCustomizations = {
        ...
        welcomeMessage:
          customizations?.welcomeMessage || "你好！有什么我可以帮助你的吗？",
      };
    ...
    return createErrorResponse(
      error.message || "Failed to create chat deployment",
      500
    );
  }
}
