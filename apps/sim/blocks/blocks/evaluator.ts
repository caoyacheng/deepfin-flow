...
export const generateEvaluatorPrompt = (
  ...
  const exampleOutput = validMetrics.reduce(
    (acc, metric) => {
      ...
      if (metric?.name) {
        acc[metric.name.toLowerCase()] = Math.floor(
          (metric.range.min + metric.range.max) / 2
        ); // Use middle of range as example
      } else {
        logger.warn(
          "Skipping invalid metric during example generation:",
          metric
        );
      }
      return acc;
...
const generateResponseFormat = (metrics: Metric[]) => {
  ...
  validMetrics.forEach((metric) => {
    ...
    if (metric?.name) {
      ...
      logger.warn(
        "Skipping invalid metric during response format property generation:",
        metric
      );
    }
  });
      ...
      value: (params: Record<string, any>) => {
        try {
          ...
          return JSON.stringify({
            systemPrompt:
              "Evaluate the content and return a JSON with metric scores.",
            responseFormat: {
              schema: {
                type: "object",
                properties: {},
