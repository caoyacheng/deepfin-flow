...
export const Mem0Block: BlockConfig<Mem0Response> = {
  ...
  subBlocks: [
    ...
    },
    {
      ...
      layout: "full",
      placeholder:
        'JSON array, e.g. [{"role": "user", "content": "I love Sim!"}]',
      language: "json",
      condition: {
        field: "operation",
        value: "add",
        ...
        switch (operation) {
            ...
            if (params.messages) {
              try {
                ...
                if (Array.isArray(messagesArray) && messagesArray.length > 0) {
                  ...
                  errors.push("Messages must be a non-empty array");
                  throw new Error(
                    "Mem0 Block Error: Messages must be a non-empty array"
                  );
                }
              } catch (e: any) {
                ...
                throw new Error(
                  `Mem0 Block Error: ${e.message || "Messages must be valid JSON"}`
                );
              }
            }
            break;
          case "search":
            if (params.query) {
              ...
              throw new Error(
                "Mem0 Block Error: Search requires a query parameter"
              );
            }
