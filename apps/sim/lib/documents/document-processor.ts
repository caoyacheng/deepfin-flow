...
export async function processDocument(
  ...
  try {
    // Parse the document
    const { content, processingMethod, cloudUrl } = await parseDocument(
      fileUrl,
      filename,
      mimeType
    );
    ...
    const tokenCount = chunks.reduce(
      (sum: number, chunk: Chunk) => sum + chunk.tokenCount,
      0
    );

    logger.info(
      `Document processed successfully: ${chunks.length} chunks, ${tokenCount} tokens`
    );
...
async function parseDocument(
  ...
  const shouldUseMistralOCR =
    mimeType === "application/pdf" && env.MISTRAL_API_KEY;
...
async function parseWithMistralOCR(
  ...
  if (!fileUrl.startsWith("https://")) {
    logger.info(
      `Uploading "${filename}" to cloud storage for Mistral OCR access`
    );
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to download file for cloud upload: ${response.statusText}`
        );
      }
      ...
      if (provider === "blob") {
        ...
        if (
          ...
          (!blobConfig.connectionString &&
            (!blobConfig.accountName || !blobConfig.accountKey))
        ) {
          throw new Error(
      ...
      try {
        ...
        cloudUrl = httpsUrl;
        logger.info(
          `Successfully uploaded to cloud storage for Mistral OCR: ${cloudResult.key}`
        );
      } catch (uploadError) {
        logger.error(
          "Failed to upload to cloud storage for Mistral OCR:",
          uploadError
        );
        throw new Error(
  ...
  try {
    ...
    if (!result.success) {
      throw new Error(
        `Mistral OCR processing failed: ${result.error || "Unknown error"}`
      );
    }
    ...
    if (fileUrl.startsWith("data:")) {
      ...
      try {
        ...
        if (mimeType === "text/plain") {
          logger.info(
            `Data URI processed successfully for text content: ${filename}`
          );
        } else {
      ...
      try {
        ...
        if (!extension) {
          throw new Error(
            `Could not determine file extension from filename: ${filename}`
          );
        }
