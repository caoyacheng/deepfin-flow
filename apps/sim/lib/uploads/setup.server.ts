...
export async function ensureUploadsDirectory() {
  if (USE_OSS_STORAGE) {
    logger.info(
      "Using Aliyun OSS storage, skipping local uploads directory creation"
    );
    return true;
  }
...
if (typeof process !== "undefined") {
  ...
  if (USE_OSS_STORAGE) {
    // Verify Aliyun OSS credentials
    if (!env.OSS_BUCKET_NAME || !env.OSS_REGION) {
      logger.warn("OSS storage configuration is incomplete");
      logger.warn("Set OSS_BUCKET_NAME and OSS_REGION for OSS storage");
    } else if (!env.OSS_ACCESS_KEY_ID || !env.OSS_ACCESS_KEY_SECRET) {
      logger.warn("OSS credentials are not set in environment variables");
      logger.warn(
        "Set OSS_ACCESS_KEY_ID and OSS_ACCESS_KEY_SECRET for OSS storage"
      );
    } else {
      logger.info("Aliyun OSS credentials found in environment variables");
      logger.info(
        `OSS bucket: ${env.OSS_BUCKET_NAME}, region: ${env.OSS_REGION}`
      );
    }
  ...
  if (USE_OSS_STORAGE && env.OSS_KB_BUCKET_NAME) {
    logger.info(`OSS knowledge base bucket: ${env.OSS_KB_BUCKET_NAME}`);
  }
  if (USE_OSS_STORAGE && env.OSS_COPILOT_BUCKET_NAME) {
    logger.info(`OSS copilot bucket: ${env.OSS_COPILOT_BUCKET_NAME}`);
  }
