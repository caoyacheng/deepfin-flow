...
describe("formatRequestParams", () => {
  ...
  it.concurrent("should handle preformatted content types", () => {
    ...
    mockTool.request.body = vi
      .fn()
      .mockReturnValue({ body: "key1=value1&key2=value2" });
...
describe("validateRequiredParametersAfterMerge", () => {
  ...
  it.concurrent(
    "should not throw error when all required parameters are provided",
    () => {
      expect(() => {
        validateRequiredParametersAfterMerge("test-tool", mockTool, {
          required1: "value",
  ...
  it.concurrent(
    "should validate mixed user-or-llm and user-only parameters correctly",
    () => {
  ...
  it.concurrent(
    "should use parameter description in error messages when available",
    () => {
  ...
  it.concurrent(
    "should fall back to parameter name when no description available",
    () => {
...
describe("executeRequest", () => {
  ...
  beforeEach(() => {
    ...
    mockTool = {
      ...
      transformError: vi.fn(
        (errorContent) => `Custom error: ${errorContent.message}`
      ),
    };
  });
  ...
  it.concurrent(
    "should use default transform response if not provided",
    async () => {
  ...
  it("should handle error responses with transformError", async () => {
    ...
    expect(mockTool.transformError).toHaveBeenCalledWith({
  ...
  it.concurrent(
    "should handle various Promise return types from transformError",
    async () => {
      // Case 1: transformError returns a Promise<string>
      mockTool.transformError = vi
        .fn()
        .mockResolvedValue("Promise string error");
      ...
      mockTool.transformError = vi
        .fn()
        .mockResolvedValue({ error: "Object error message" });
...
describe("createParamSchema", () => {
  it.concurrent(
    "should create parameter schema from custom tool schema",
    () => {
      const customTool = {
        id: "test-tool",
        title: "Test Tool",
...
describe("getClientEnvVars", () => {
  it.concurrent(
    "should return environment variables from store in browser environment",
    () => {
...
describe("createCustomToolRequestBody", () => {
  it.concurrent(
    "should create request body function for client-side execution",
    () => {
      const customTool = {
        code: "return a + b",
        schema: {
      ...
      const bodyFn = createCustomToolRequestBody(
        customTool,
        true,
        undefined,
        mockStoreGetter
  ...
  it.concurrent(
    "should create request body function for server-side execution",
    () => {
      const customTool = {
        code: "return a + b",
        schema: {
